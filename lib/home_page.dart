...
class _HomePageState extends State<HomePage> {
  ...
  void playGame(GameButton gb) {
    setState(() {
      if (activePlayer == 1) {
        gb.text = "X";
        gb.bg = Colors.yellow[800];
        activePlayer = 2;
        player1.add(gb.id);
      } else {
        gb.text = "0";
        gb.bg = Colors.orange[800];
        activePlayer = 1;
        player2.add(gb.id);
      }
      gb.enabled = false;
  ...
  int checkWinner() {
    ...
    if (winner != -1) {
      if (winner == 1) {
        showDialog(
            context: context,
            builder: (_) => new CustomDialog("Congratulations! You won",
                "Press Restart to start again", resetGame));
      } else {
        showDialog(
            context: context,
            builder: (_) => new CustomDialog(
                "Device Won!", "Press Restart to start again", resetGame));
      }
    }
    return winner;
  }
